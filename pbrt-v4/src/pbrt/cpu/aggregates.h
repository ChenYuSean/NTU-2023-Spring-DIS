...
struct Voxel {
  public:
    Voxel() {}
    void AddPrimitive(Primitive &prim) { primitives_ptr.push_back(&prim); }
...
// GridAggregate Declarations
class GridAggregate {
  public:
    GridAggregate(std::vector<Primitive> prims);
  ...
  private:
    // GridAggregate Private Data
    Bounds3f bounds;                    // Bounds of all primitives
    std::vector<Primitive> primitives;  // primitives inside the grid
    std::vector<Voxel> voxels;
    int nVoxels[3];
    int width[3];     // x,y,z width of voxel
    int invWidth[3];  // inverse of width
    ...
    int posToVoxel(const Point3f &P, int axis) const {
        ...
        int v = static_cast<int>((val - this->bounds.pMin[axis]) * invWidth[axis]);
