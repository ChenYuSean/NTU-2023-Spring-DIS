...
GridAggregate::GridAggregate(std::vector<Primitive> prims)
    ...
    uint32_t primSize = static_cast<uint32_t>(primitives.size());
    // Compute bounds and choose grid resolution
    for (uint32_t i = 0; i < primSize; ++i)
        this->bounds = Union(this->bounds, primitives[i].Bounds());
    ...
    for (int axis = 0; axis < 3; ++axis) {
        this->nVoxels[axis] = std::ceil(delta[axis] * voxelsPerUnitDist);
        this->nVoxels[axis] = Clamp(this->nVoxels[axis], 1, 64);
    }

    // Compute voxel widths and allocate voxels
    for (int axis = 0; axis < 3; ++axis) {
        this->width[axis] = delta[axis] / this->nVoxels[axis];
        this->invWidth[axis] = (this->width[axis] == 0.f) ? 0.f : 1.f / this->width[axis];
    }
    int nv = this->nVoxels[0] * this->nVoxels[1] * this->nVoxels[2];
    this->voxels.resize(nv);
        ...
        for (int z = vmin[2]; z <= vmax[2]; ++z)
            for (int y = vmin[1]; y <= vmax[1]; ++y)
                for (int x = vmin[0]; x <= vmax[0]; ++x) {
                    int o = offset(x, y, z);
                    this->voxels[o].AddPrimitive(primitives[i]);
                }
    }
}
...
GridAggregate *GridAggregate::Create(std::vector<Primitive> prims,
    ...
    DBG_STR("Create Grid Aggregator");
...
pstd::optional<ShapeIntersection> GridAggregate::Intersect(const Ray &ray,
    ...
    // Exit if there is no intersect of all bounds
    if (!this->bounds.IntersectP(ray.o, ray.d, tMax)) {
        return {};
    }
    ...
    for(;;){
        ...
        for (int axis = 1; axis < 3; ++axis) {
            if (tNear[axis] < tNear[target_axis]) {
                target_axis = axis;
            }
        }
