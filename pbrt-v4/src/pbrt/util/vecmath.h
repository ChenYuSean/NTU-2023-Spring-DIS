...
PBRT_CPU_GPU inline bool IsNaN(Interval fi) { return pbrt::IsNaN(Float(fi)); }
...
template <> struct TupleLength<Interval> { using type = Interval; };
...
PBRT_CPU_GPU inline auto operator*(U s, Tuple2<C, T> t)
    -> C<decltype(T{} * U{})> {
  DCHECK(!t.HasNaN());
  return t * s;
}
...
template <template <class> class C, typename T, typename U>
PBRT_CPU_GPU inline auto operator*(U s, Tuple3<C, T> t)
    -> C<decltype(T{} * U{})> {
  return t * s;
}
...
public:
  ...
  template <typename U> PBRT_CPU_GPU explicit Vector2(Point2<U> p);
  template <typename U>
...
public:
  ...
  template <typename U> PBRT_CPU_GPU explicit Vector3(Normal3<U> n);
};
...
public:
  ...
  template <typename U>
  PBRT_CPU_GPU auto operator+(Vector2<U> v) const
      -> Point2<decltype(T{} + U{})> {
    DCHECK(!v.HasNaN());
    return {x + v.x, y + v.y};
  }
  template <typename U> PBRT_CPU_GPU Point2<T> &operator+=(Vector2<U> v) {
    DCHECK(!v.HasNaN());
    x += v.x;
    y += v.y;
    return *this;
  ...
  template <typename U>
  PBRT_CPU_GPU auto operator-(Point2<U> p) const
...
PBRT_CPU_GPU inline Point2f InvertBilinear(Point2f p,
                                           pstd::span<const Point2f> v);

// https://www.iquilezles.org/www/articles/ibilinear/ibilinear.htm,
// with a fix for perfect quads
PBRT_CPU_GPU inline Point2f InvertBilinear(Point2f p,
                                           pstd::span<const Point2f> vert) {
...
public:
  ...
  template <typename U> PBRT_CPU_GPU Point3fi operator+(Vector3<U> v) const {
    DCHECK(!v.HasNaN());
    return {x + v.x, y + v.y, z + v.z};
  }
  template <typename U> PBRT_CPU_GPU Point3fi &operator+=(Vector3<U> v) {
    DCHECK(!v.HasNaN());
    x += v.x;
    y += v.y;
    z += v.z;
  ...
  template <typename U> PBRT_CPU_GPU Point3fi operator-(Point3<U> p) const {
    DCHECK(!p.HasNaN());
    return {x - p.x, y - p.y, z - p.z};
  }
  template <typename U> PBRT_CPU_GPU Point3fi operator-(Vector3<U> v) const {
...
template <typename T>
PBRT_CPU_GPU inline auto LengthSquared(Vector2<T> v) ->
    typename TupleLength<T>::type {
...
template <typename T> PBRT_CPU_GPU inline auto Normalize(Vector2<T> v) {
  return v / Length(v);
}
...
template <typename T> PBRT_CPU_GPU inline auto Normalize(Vector3<T> v) {
  return v / Length(v);
}

template <typename T> PBRT_CPU_GPU inline T Dot(Vector3<T> v, Vector3<T> w) {
...
template <typename T>
PBRT_CPU_GPU inline void CoordinateSystem(Vector3<T> v1, Vector3<T> *v2,
                                          Vector3<T> *v3) {
...
template <typename T>
PBRT_CPU_GPU inline void CoordinateSystem(Normal3<T> v1, Vector3<T> *v2,
                                          Vector3<T> *v3) {
  Float sign = pstd::copysign(Float(1), v1.z);
  Float a = -1 / (sign + v1.z);
  Float b = v1.x * v1.y * a;
...
template <typename T> PBRT_CPU_GPU inline auto Normalize(Normal3<T> n) {
  return n / Length(n);
}
...
PBRT_CPU_GPU inline Float Length(Quaternion q) { return std::sqrt(Dot(q, q)); }
...
public:
  ...
  PBRT_CPU_GPU
  Slab3() {
    normal = Normal3<T>(1, 0, 0);
    dMin = 0;
    dMax = 0;
  }

  PBRT_CPU_GPU
  Slab3(Normal3<T> n, T d1, T d2) {
    normal = n;
    dMin = std::min(d1, d2);
    dMax = std::max(d1, d2);
  }
  ...
  PBRT_CPU_GPU
  explicit Slab3(Point3<T> p, T d1, T d2) {
    normal = Normal3<T>(p);
    dMin = std::min(d1, d2);
    dMax = std::max(d1, d2);
  }
  ...
  PBRT_CPU_GPU
  explicit Slab3(Vector3<T> v, T d1, T d2) {
    normal = Normal3<T>(v);
    dMin = std::min(d1, d2);
    dMax = std::max(d1, d2);
  }
  ...
  PBRT_CPU_GPU
  bool IsEmpty() const { return dMin >= dMax; }
  PBRT_CPU_GPU
  bool IsDegenerate() const { return dMin > dMax; }

  template <typename U> PBRT_CPU_GPU explicit Slab3(const Slab3<U> &s) {
    if (s.IsEmpty())
  ...
  PBRT_CPU_GPU
  bool operator==(const Slab3<T> &s) const {
  ...
  std::string ToString() const {
    return StringPrintf("(%s,%s,%s): [ %s - %s ]", normal.x, normal.y, normal.z,
                        dMin, dMax);
  }
...
public:
  ...
  template <typename U> PBRT_CPU_GPU explicit Bounds2(const Bounds2<U> &b) {
    if (b.IsEmpty())
      // Be careful about overflowing float->int conversions and the
      // like.
      *this = Bounds2<T>();
...
template <typename T>
PBRT_CPU_GPU inline Bounds2<T> Union(const Bounds2<T> &b1,
                                     const Bounds2<T> &b2) {
...
template <typename T>
PBRT_CPU_GPU inline Bounds2<T> Intersect(const Bounds2<T> &b1,
                                         const Bounds2<T> &b2) {
  // Be careful to not run the two-point Bounds constructor.
  Bounds2<T> b;
...
template <typename T>
PBRT_CPU_GPU inline Bounds3<T> Union(const Bounds3<T> &b1,
                                     const Bounds3<T> &b2) {
  Bounds3<T> ret;
  ret.pMin = Min(b1.pMin, b2.pMin);
  ret.pMax = Max(b1.pMax, b2.pMax);
...
PBRT_CPU_GPU inline Float SphericalTriangleArea(Vector3f a, Vector3f b,
                                                Vector3f c) {
...
PBRT_CPU_GPU inline Float SphericalTheta(Vector3f v) { return SafeACos(v.z); }
...
PBRT_CPU_GPU inline Float CosTheta(Vector3f w) { return w.z; }
PBRT_CPU_GPU inline Float Cos2Theta(Vector3f w) { return Sqr(w.z); }
PBRT_CPU_GPU inline Float AbsCosTheta(Vector3f w) { return std::abs(w.z); }
...
PBRT_CPU_GPU
inline bool SameHemisphere(Vector3f w, Normal3f wp) { return w.z * wp.z > 0; }
...
PBRT_CPU_GPU inline DirectionCone BoundSubtendedDirections(const Bounds3f &b,
                                                           Point3f p) {
  ...
  return cosTheta * w +
         (sinTheta / Length(a)) *
             Vector3f(
                 ...
                 w.z * (wp.x * w.x + wp.y * w.y) -
                     wp.z * (Sqr(w.x) + Sqr(w.y)));
}
